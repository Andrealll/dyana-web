...
function getSupabaseRedirectUrl() {
  // 1️⃣ Se è configurata la env, la usiamo così com'è
  //    (deve essere del tipo: https://dyana-web.vercel.app/reset_password)
  if (SUPABASE_REDIRECT_FALLBACK) {
    return SUPABASE_REDIRECT_FALLBACK;
  }

  // 2️⃣ In browser: origin + /reset_password (underscore, come la pagina Next)
  if (typeof window !== "undefined") {
    return `${window.location.origin}/reset_password`;
  }

  // 3️⃣ Ultimo fallback: localhost (dev)
  return "http://localhost:3000/reset_password";
}
