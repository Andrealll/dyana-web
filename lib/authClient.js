...
function resolveBase(envValue, port) {
  // SSR: niente window → usiamo env o localhost
  if (typeof window === "undefined") {
    return envValue ?? `http://127.0.0.1:${port}`;
  }

  const host = window.location.hostname;

  // Se sto sviluppando da PC (localhost / 127.0.0.1),
  // uso l'env se esiste, altrimenti faccio fallback su host:port
  if (host === "localhost" || host === "127.0.0.1") {
    return envValue ?? `http://${host}:${port}`;
  }

  // Se sto aprendo da cellulare / altro device (es. 172.20.10.2),
  // uso SEMPRE quell'host per raggiungere i backend in LAN.
