...
if (typeof window !== "undefined") {
  ...
  console.log("[AUTH CONFIG][window] AUTH_BASE =", window.__AUTH_BASE);
}
...
// ============================
// CAPACITOR NATIVE STORAGE (ADD-ON, NON ROMPE WEB)
// ============================
// Obiettivo: persistenza token anche se WebView/localStorage vengono ricreati su Android.
// - NON rimuoviamo localStorage.
// - Su native scriviamo/leggiamo ANCHE da Preferences.
// - Usiamo import dinamico per non rompere build web.

let __isNative = false;
let __prefs = null;

// cache in memoria per API sync (getToken/getGuestToken sono sync)
let __tokenCache = null;
let __guestTokenCache = null;
let __hydratePromise = null;

async function initNativeStorageOnce() {
  if (typeof window === "undefined") return false;
  if (__prefs) return true;

  try {
    const cap = await import("@capacitor/core");
    __isNative = !!cap?.Capacitor?.isNativePlatform?.();
    if (!__isNative) return false;

    const pref = await import("@capacitor/preferences");
    __prefs = pref?.Preferences || null;
    return !!__prefs;
  } catch (e) {
    // Se i pacchetti non esistono in web build, non deve esplodere.
    __isNative = false;
    __prefs = null;
    return false;
  }
...
async function hydrateNativeTokensOnce() {
  if (typeof window === "undefined") return;

  if (__hydratePromise) return __hydratePromise;

  __hydratePromise = (async () => {
    const ok = await initNativeStorageOnce();
    if (!ok || !__prefs) return;

    try {
      const t = await __prefs.get({ key: TOKEN_KEY });
      const g = await __prefs.get({ key: GUEST_TOKEN_KEY });
      __tokenCache = t?.value ?? __tokenCache ?? null;
      __guestTokenCache = g?.value ?? __guestTokenCache ?? null;

      // Debug utile (puoi togliere dopo)
      console.log("[AUTH][NATIVE] hydrated token cache:", {
        hasUser: !!__tokenCache,
        hasGuest: !!__guestTokenCache,
      });
    } catch (e) {
      console.warn("[AUTH][NATIVE] hydrate error:", e);
    }
  })();
...
// Avvio hydration non bloccante (NON cambia comportamento web)
if (typeof window !== "undefined") {
  // fire-and-forget
  hydrateNativeTokensOnce().catch(() => {});
}

function nativeSetTokenFireAndForget(key, value) {
  // Non blocchiamo chiamanti sync.
  hydrateNativeTokensOnce()
    .then(async () => {
      if (!__prefs) return;
      await __prefs.set({ key, value: String(value ?? "") });
    })
    .catch(() => {});
}

function nativeRemoveTokenFireAndForget(key) {
  hydrateNativeTokensOnce()
    .then(async () => {
      if (!__prefs) return;
      await __prefs.remove({ key });
    })
    .catch(() => {});
}
...
export function saveToken(token) {
  ...
  // ADD-ON: cache + native persistence
  __tokenCache = token ?? null;
  __guestTokenCache = null;

  nativeSetTokenFireAndForget(TOKEN_KEY, token);
  nativeRemoveTokenFireAndForget(GUEST_TOKEN_KEY);
}

export function getToken() {
  if (typeof window === "undefined") return null;

  // 1) localStorage (comportamento originale)
  try {
  ...
  // 2) cache nativa (sync)
  if (__tokenCache) return __tokenCache;
...
export function clearToken() {
  ...
  __tokenCache = null;
  nativeRemoveTokenFireAndForget(TOKEN_KEY);
}
...
export function getGuestToken() {
  ...
  // 1) localStorage (originale)
  try {
    const g = localStorage.getItem(GUEST_TOKEN_KEY);
    if (g) {
      __guestTokenCache = g;
      return g;
    }
  ...
  // 2) cache nativa (sync)
  if (__guestTokenCache) return __guestTokenCache;
...
export function saveGuestToken(token) {
  ...
  __guestTokenCache = token ?? null;
  nativeSetTokenFireAndForget(GUEST_TOKEN_KEY, token);
}
...
export async function ensureGuestToken() {
  ...
  // ADD-ON: prova a idratare prima di decidere
  try { await hydrateNativeTokensOnce(); } catch {}
...
export async function getAnyAuthTokenAsync() {
  // ADD-ON: idrata prima per mobile
  try { await hydrateNativeTokensOnce(); } catch {}
...
export async function exchangeSupabaseTokenForDyanaJwt(supabaseAccessToken) {
  ...
  if (!res.ok) {
    console.error("[AUTH][EXCHANGE] error:", res.status, txt || "<no body>");
